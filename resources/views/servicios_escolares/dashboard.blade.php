<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-utgz-primary leading-tight">
            {{ __('Dashboard Servicios Escolares') }}
        </h2>
    </x-slot>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 fade-in-up">
        <div class="bg-white rounded-xl shadow-sm p-6 flex items-center transition-transform hover:scale-105">
            ...
            <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-utgz-primary">{{ count($documentosPendientes) }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 flex items-center transition-transform hover:scale-105">
            ...
            <div class="ml-4">
                <h3 class="text-gray-500 text-sm font-medium">Aprobados</h3>
                <p class="text-2xl font-bold text-green-600">{{ count($documentosAprobados) }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 flex items-center transition-transform hover:scale-105">
            ...
            <div class="ml-4">
                <h3 class="text-gray-500 text-sm font-medium">Rechazados</h3>
                <p class="text-2xl font-bold text-red-600">{{ count($documentosRechazados) }}</p>
            </div>
        </div>
    </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-gray-600">{{ $doc->etapa->nombre }}</td>
                            ...
                            <td class="px-6 py-4 text-right flex justify-end gap-2">
                                ...
                                <form action="{{ route('servicios_escolares.documentos.revisar', $doc->id) }}" method="POST" class="inline reject-action">
                                    @csrf
